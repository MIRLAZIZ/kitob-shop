<template>
  <div>
    <div class="container px-0">
      ...
      <!-- loading  -->
      <div class="d-flex align-items-center flex-column my-5" style="height: 500px; justify-content: center"  v-if="loading">
      ...
      <div v-else>
        <!-- empty  -->
        <div v-if="!basketLength" class="d-flex align-items-center flex-column my-5">
          <img src="/assets/contact/basketEmpty.png" alt="" />
          <h4 style="font-weight: 600">{{ $t('order.Cart_empty') }}</h4>
          <small @click="$router.push('/')">
            <small style="color: #1c5793; cursor: pointer;"> {{ $t('home.profile.Homepage') }}</small>
        ...
        <div v-else>
          ...
          <div class="mt-4 mb-3">
            ...
            <small class="basketCount">{{ basketLength }} {{ $t("home.basket.product") }}</small>
          </div>
          ...
          <div class="row mb-5">
            <div class="col-9">
              <div class="d-flex justify-content-between mb-3">
                <div class="d-flex align-items-center">
                  <input type="checkbox" class="basketCheck" v-model="checkAll" @click="selectAll" />
              ...
              <div class="d-flex mt-4" v-for="(item, idx) in store.basket" :key="idx">
                <div class="me-3 d-flex align-items-center">
                  <input type="checkbox" class="basketCheck" v-model="item.product.is_check" />
                </div>
                ...
                <div class="basketProduts">
                  <div class="d-flex justify-content-between">
                    <div class="d-flex">
                      <div>
                        <img :src="urlimg + '/' + item.product.image" alt="" class="basketImg" />
                      </div>

                      <div class="ms-4 ps-2">
                        <p class="bookTitle">{{ item.product.name }}</p>
                        <p class="bookAuthor">{{ item.product.author }}</p>
                        <p class="bookPrice">{{ item.product.price }} {{ $t("home.basket.sum") }}</p>
                        <div class="basket">

                          <!-- types  -->
                          <button class="btn border px-3" v-for="(itm, index) in item.product.types" :key="index"
                    ...
                    <div class="d-flex flex-column justify-content-between">
                      <div>
                        <div class="d-flex">
                          <div class="basketLike" @click="
                            ...
                            ">
                            <svg width="22" height="22" viewBox="0 0 22 22" xmlns="http://www.w3.org/2000/svg" :style="{
                          ...
                          <div class="basketdelate" @click="deleteBasket(item.id)">
                            <img class="imgDelate" src="/assets/contact/basketdelate.png" alt="" />
                            <img class="basketDelateHover" src="/assets/contact/basketDelateHover.png" alt="" />
                          </div>
                        </div>
                      </div>
                      ...
                      <div class="productCount">
                        ...
                        <button class="btn" @click="productRemove(idx)"
                        ...
                        <button class="btn" @click="productAdd(idx)"
